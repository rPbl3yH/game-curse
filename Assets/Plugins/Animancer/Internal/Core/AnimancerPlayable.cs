...
namespace Animancer
{
        ...
        IEnumerator, IPlayableWrapper, IAnimationClipCollection
    {
        ...
        /// <remarks>
        /// You can use <see cref="AnimancerNode.Graph"/> if you need to access the graph in nodes defined outside the
        /// Animancer assembly.
        /// </remarks>
        ...
        {
            if (_Graph.IsValid())
                return "AnimancerPlayable (" + _Graph.GetEditorName() + ")";
            else
                return "AnimancerPlayable (Graph Not Initialised)";
        }
#endif
        ...
        {
            var index = Key.IndexOf(((IKeyHolder)node).Key);
            if (index < 0)
                return;
            ...
            try
            {
                while (--_CurrentNode >= 0)
                {
                    ...
                    if (node._Playable.IsValid())
                    {
                        ...
                        node.Update(out needsMoreUpdates);
                        if (needsMoreUpdates)
                            continue;
                    }
