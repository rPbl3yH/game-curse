...
namespace Animancer
{
    ...
    public abstract partial class AnimancerState : AnimancerNode, IAnimationClipCollection
    {
        ...
        public sealed override IPlayableWrapper Parent { get { return _Parent; } }
        ...
        public override AnimancerLayer Layer { get { return _Parent != null ? _Parent.Layer : null; } }
...
#if UNITY_ASSERTIONS
                if (!value.IsFinite())
                    throw new ArgumentOutOfRangeException("value", value, "Time must be finite");
#endif

                var root = Root;
                if (root == null || _TimeFrameID == root.FrameID)
                {
                    if (_Time == value)
                        return;
                }
        ...
        {
            text.Append(delimiter).Append("Key: ").Append(AnimancerUtilities.ToStringOrNull(_Key));

            var mainObject = MainObject;
            if (mainObject != _Key as Object)
                text.Append(delimiter).Append("MainObject: ").Append(AnimancerUtilities.ToStringOrNull(mainObject));
