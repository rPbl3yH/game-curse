...
namespace Animancer
{
    ...
    public sealed class AnimancerLayer : AnimancerNode, IAnimationClipCollection
    {
        ...
        /// </summary>
        /// <exception cref="ArgumentException">
        /// Thrown if the <see cref="AnimancerState.Clip"/> is null when using <see cref="FadeMode.FromStart"/> or
                ...
                case FadeMode.FromStart:
                    {
                        ...
                        if (state.Clip == null)
                            throw new ArgumentException(
                                "FadeMode.FromStart can only be used on ClipStates. State = " + state);
#endif
                ...
                case FadeMode.NormalizedFromStart:
                    {
                        ...
                        if (state.Clip == null)
                            throw new ArgumentException(
                                "FadeMode.NormalizedFromStart can only be used on ClipStates. State = " + state);
#endif
        ...
        /// </summary>
        public static int MaxStateDepth { get; private set; }

        static AnimancerLayer()
        {
            MaxStateDepth = 5;
        }
#endif
            ...
            if (state.Weight != 0)
            {
                ...
                if (clip == null)
                {
                    ...
                    throw new InvalidOperationException(
                        "GetOrCreateWeightlessState can only be used on ClipStates. State = " + state);
                }
                ...
                while (state.Weight != 0)
                {
                    ...
                    if (++depth == MaxStateDepth)
                    {
                        throw new ArgumentOutOfRangeException("depth", "GetOrCreateWeightlessState has created " +
                            ...
                            " You probably just want to use FadeMode.FixedSpeed instead, but you can call" +
                            " AnimancerLayer.SetMaxStateDepth if necessary.");
                    }
#endif
                }
            }
        ...
        /// method will automatically be compiled out of a runtime build. <see cref="ToString"/> returns the name.
        /// </summary>
